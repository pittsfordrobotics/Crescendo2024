...
public class AutoCommandFactory {
    ...
    public AutoCommandFactory(SwerveSubsystem swerveSubsystem) {
        ...
        xPIDController = new PIDController(0, 0.0, 0.0); // PIDController for field-relative X translation (input: Y error in meters, output: m/s).
        yPIDController = new PIDController(0, 0, 0); // PIDController for field-relative Y translation (input: Y error in meters, output: m/s).
        headinganglePIDController = new PIDController(0, 0.0, 0.0); // PID controller to correct for rotation error
